...
void main() {
	...
	vec3 ambient_lighting = albedo.rgb * ambient_factor;
	vec3 diffuse_lighting = albedo.rgb * max(dot(N, light_dir), 0.0) * light_color * shadow;
	out_color = vec4(ambient_lighting + diffuse_lighting, 1.0);
}
