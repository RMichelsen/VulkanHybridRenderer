...
layout(location = 3) out vec4 out_motion_vectors_and_fragment_depth;
...
void main() {
	...
	out_pos = vec4(in_world_space_pos.xyz, roughness);
	vec3 N = in_normal;
	...
	out_normal = vec4(vec3_encode_to_oct(normalize(mat3(pc.normal_matrix) * N)), pc.object_id, metallic);
	out_albedo = albedo;
